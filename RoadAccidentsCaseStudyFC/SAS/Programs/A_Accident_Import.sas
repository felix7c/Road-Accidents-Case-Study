...
*import weather conditions format;
proc import 
datafile="&data_location.\variable lookup (1).xls"
out=work.weather_fmt 
dbms=xls 
  replace;
  sheet="Weather";
  getnames=YES; 
  datarow=2;
  range="Weather$A1:B11";
run;



*creating format dataset;
data shared.weather_fmt;
  retain fmtname 'weather_fmt' type 'N'; *format name, type =num;
  set work.weather_fmt (rename=(code=start));
run;

*creating the format from the dataset;
proc format library=shared cntlin=shared.weather_fmt;
run;
...
*import accident severity format;
proc import 
datafile="&data_location.\variable lookup (1).xls"
out=work.severity_fmt 
dbms=xls 
  replace;
  sheet="Accident Severity";
  getnames=YES; 
  datarow=2;
  range="Accident Severity$A1:B4";
run;



*creating format dataset;
data shared.severity_fmt;
  retain fmtname 'severity_fmt' type 'N'; *format name, type =num;
  set work.severity_fmt (rename=(code=start));
run;

*creating the format from the dataset;
proc format library=shared cntlin=shared.severity_fmt;
run;
...
dbms=xls 
  ...
  getnames=YES; 
  datarow=2;
  range="Vehicle Type$A1:B22";
run;



*creating format dataset;
data shared.veh_t_fmt;
  retain fmtname 'veh_t_fmt' type 'N'; *format name, type =num;
  set work.veh_t_fmt (rename=(code=start));
run;

*creating the format from the dataset;
proc format library=shared cntlin=shared.veh_t_fmt;
run;
